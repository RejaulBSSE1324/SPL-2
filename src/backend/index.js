...
app.post('/seats', async (req, res) => {
    ...
    try {
        ...
        // Handle duplicate key error (MongoError 11000)
        if (error.code === 11000 && error.keyPattern && error.keyPattern.seatNumber === 1 && error.keyPattern.roomNumber === 1 && error.keyPattern.buildingName === 1) {
            res.status(400).json({ error: 'Seat with same details already exists' });
        } else {
...
// Fetch all seats with populated student field
app.get('/seats', async (req, res) => {
    try {
        const seats = await Seat.find({}).populate('student');
...
// Route to fetch seat ID by details
app.get('/seats/details', async (req, res) => {
    const { seatNumber, roomNumber, buildingName } = req.query;
  
    try {
      const seat = await Seat.findOne({ seatNumber, roomNumber, buildingName });
...
app.delete('/seats/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Seat removed successfully' });
    } catch (error) {
...
// Allocate a student to a seat
app.post('/seats/allocate', async (req, res) => {
    console.log(req.body)
    try {
        const { seatId, registrationNumber, studentName } = req.body;
        const seat = await Seat.findById(seatId);

        if (seat.student && seat.student.name) {
            return res.status(400).json({ message: "Seat already allocated" });
        }

        seat.student = { name: studentName, reg: registrationNumber };
        ...
        res.status(200).json({ seat, application });
    } catch (err) {
        console.error(err.message);
        res.status(500).send("Server error");
    }
...
// Deallocate a student from a seat
app.post('/seats/deallocate', async (req, res) => {
    try {
